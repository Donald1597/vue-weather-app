<template>
  ...
  >
    <div
      class="flex flex-col sm:flex-row justify-between items-center mb-8 space-y-4 sm:space-y-0 w-full"
    >
      <div class="flex justify-between items-center w-full sm:w-auto">
        <h1
          class="text-3xl sm:text-4xl md:text-5xl text-teal-300 font-bold transition-transform transform hover:scale-110 dark:text-teal-500"
        >
          Weather App
        </h1>
        <div class="flex items-center sm:hidden ml-4">
          <input
            type="checkbox"
            id="theme-toggle-small"
            class="hidden"
            :checked="darkModeStore.isDarkMode"
            @change="toggleDarkMode"
          />
          <label
            for="theme-toggle-small"
      ...
      >
        ...
        <button
          ...
          class="bg-teal-500 text-white rounded-lg px-4 py-2 sm:px-6 sm:py-3 hover:bg-teal-400 transition-transform transform hover:scale-105 dark:bg-teal-400 dark:hover:bg-teal-300 w-full sm:w-auto"
        >
          Search
        </button>
